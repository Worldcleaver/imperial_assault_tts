...
function buttonPress()
    ...
    if retract == false then
        ...
        for t, item in ipairs(Data) do
            if item.getDescription() == 'Tier 1 Items' and string.sub(item.getPosition().x,1,3) == '-65' then
        ...
        for i=0, half-1 do
            local params ={}
            params.position = {25-(i*4),1.5, 26.23}
            params.flip = true
      ...
      local Data = getAllObjects()
      for t, item in ipairs(Data) do
          if item.getDescription() == 'Tier 1 Item' and item.getPosition().x > -46 and item.getPosition().x < 46 and item.getPosition().z > -31 and item.getPosition().z < 31  then
            deck.putObject(item)
          end
      end
      retract = false
...
function onload(save_state)
  if save_state ~= nil and save_state ~= "" then
    ...
    self.createButton({
        label="Rebel upgrade stage button", click_function = "buttonPress",
