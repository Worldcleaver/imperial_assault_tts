  ...
  if startingsize == nil then
    ...
    if retract == false then
        ...
        for t, item in ipairs(Data) do
            if item.getDescription() == 'Tier 3 Items' and string.sub(item.getPosition().x,1,3) == '-65' then
        ...
        local Data = getAllObjects()
        for t, item in ipairs(Data) do
            if item.getDescription() == 'Tier 3 Item' and item.getPosition().x > -46 and item.getPosition().x < 46 and item.getPosition().z > -31 and item.getPosition().z < 31  then
              deck.putObject(item)
            end
        end
        retract = false
